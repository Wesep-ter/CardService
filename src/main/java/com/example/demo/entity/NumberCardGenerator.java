...
public class NumberCardGenerator {
    ...
    public String generateNumberCard(PaymentSystem paymentSystem, Long userId){
        if (BIN.length() != 5 || !BIN.matches("\\d+")){
            throw new RuntimeException("BIN должен состоять из 5 цифр.");
        }
        String first6 = paymentSystem.getNum() + BIN;
        String clientSegment = generateClientSegment(userId);
        if (clientSegment == null){
            throw new RuntimeException("Ошибка генерации clientIdSegment");
        }
    ...
    private String generateClientSegment(Long userId) {
        String input = BIN + ":" + userId;
